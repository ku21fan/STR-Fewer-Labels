...
def train(opt, log):
    ...
    if opt.select_data == 'label':
        select_data = ['1.SVT', '2.IIIT', '3.IC13', '4.IC15', '5.COCO',
                       '6.RCTW17', '7.Uber', '8.ArT', '9.LSVT', '10.MLT19', '11.ReCTS']
        ...
        select_data = ['1.SVT', '2.IIIT', '3.IC13', '4.IC15', '5.COCO',
                       '6.RCTW17', '7.Uber', '8.ArT', '9.LSVT', '10.MLT19', '11.ReCTS', 'MJ', 'ST']

    elif opt.select_data == 'mix_SA':
        select_data = ['1.SVT', '2.IIIT', '3.IC13', '4.IC15', '5.COCO',
                       '6.RCTW17', '7.Uber', '8.ArT', '9.LSVT', '10.MLT19', '11.ReCTS', 'MJ', 'ST', 'SA']
    ...
    train_loader = Batch_Balanced_Dataset(opt, opt.train_data, select_data, batch_ratio, log)

    if opt.semi != 'None':
        select_data_unlabel = ['U1.Book32', 'U2.TextVQA', 'U3.STVQA']
        batch_ratio_unlabel = [round(1 / len(select_data_unlabel), 3)] * len(select_data_unlabel)
        dataset_root_unlabel = 'data_CVPR2021/training/unlabel/'
        train_loader_unlabel_semi = Batch_Balanced_Dataset(
            opt, dataset_root_unlabel, select_data_unlabel, batch_ratio_unlabel, log, learn_type='semi')

    AlignCollate_valid = AlignCollate(opt, mode='test')
    valid_dataset, valid_dataset_log = hierarchical_dataset(root=opt.valid_data, opt=opt, mode='test')
    ...
    for iteration in tqdm(range(start_iter + 1, opt.num_iter + 1), total=opt.num_iter, position=0, leave=True):
        ...
        labels_index, labels_length = converter.encode(labels, batch_max_length=opt.batch_max_length)
        ...
        if 'Pseudo' in opt.semi:
            ...
            image_tensors_unlabel, image_tensors_unlabel_ema = train_loader_unlabel_semi.get_batch_two_images()
            ...
            loss_SemiSL = criterion_SemiSL(student_input=student_input, student_logit=preds, student=model,
                                           teacher_input=teacher_input, iteration=iteration)
        ...
        if iteration % opt.val_interval == 0 or iteration == 1:
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                ...
                opt.writer.add_scalar('train/train_loss', float(f'{train_loss_avg.val():0.5f}'), iteration)
                opt.writer.add_scalar('train/semi_loss', float(f'{semi_loss_avg.val():0.5f}'), iteration)
                opt.writer.add_scalar('train/lr', float(f'{lr:0.7f}'), iteration)
                opt.writer.add_scalar('train/elapsed_time', float(f'{elapsed_time:0.1f}'), iteration)
                opt.writer.add_scalar('valid/valid_loss', float(f'{valid_loss:0.5f}'), iteration)
                opt.writer.add_scalar('valid/current_score', float(f'{current_score:0.2f}'), iteration)
                opt.writer.add_scalar('valid/best_score', float(f'{best_score:0.2f}'), iteration)
    ...
    opt.writer.add_scalar('test/total_accuracy', float(f'{total_accuracy:0.2f}'), iteration)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data', default='data_CVPR2021/training/label/',
    ...
    parser.add_argument('--Aug', type=str, default='None', help='whether to use augmentation |None|Blur|Crop|Rot|')
    ...
    print(f'Command line input: CUDA_VISIBLE_DEVICES={opt.CUDA_VISIBLE_DEVICES} python {command_line_input}')
    log.write(f'Command line input: CUDA_VISIBLE_DEVICES={opt.CUDA_VISIBLE_DEVICES} python {command_line_input}\n')
