...
class PseudoLabelLoss(nn.Module):
    ...
    def __init__(self, opt, converter, criterion):
        ...
        self.PseudoLabel_prediction_model = torch.nn.DataParallel(self.PseudoLabel_prediction_model).to(device)
        print(f'### loading pretrained model for PseudoLabel from {opt.model_for_PseudoLabel}')
        self.PseudoLabel_prediction_model.load_state_dict(torch.load(opt.model_for_PseudoLabel))
    ...
    def forward(self, image_unlabel, model):
        ...
        with torch.no_grad():
            if 'CTC' in self.opt.Prediction:
                ...
                idx_for_pred = torch.LongTensor(image_unlabel.size(0)).fill_(self.opt.sos_token_index).to(device)
                PseudoLabel_pred = self.PseudoLabel_prediction_model(image_unlabel, text=idx_for_pred, is_train=False)

        _, PseudoLabel_index = PseudoLabel_pred.max(2)
        length_for_decode = torch.IntTensor([PseudoLabel_pred.size(1)] * PseudoLabel_pred.size(0))
        ...
        for image_ul, pseudo_label in zip(image_unlabel_unbind, PseudoLabel_tmp):
            ...
            if '[PAD]' in pseudo_label or '[UNK]' in pseudo_label or '[SOS]' in pseudo_label:
                continue
            else:
        ...
        Pseudo_index, Pseudo_length = self.converter.encode(PseudoLabel, batch_max_length=self.opt.batch_max_length)
        ...
        if 'CTC' in self.opt.Prediction:
            ...
            loss_SemiSL = self.criterion(preds_PL_log_softmax, Pseudo_index, preds_PL_size, Pseudo_length)
        else:
            preds_PL = model(image_unlabel_filtered, Pseudo_index[:, :-1])  # align with Attention.forward
            target = Pseudo_index[:, 1:]  # without [SOS] Symbol
            loss_SemiSL = self.criterion(preds_PL.view(-1, preds_PL.shape[-1]), target.contiguous().view(-1))
...
class MeanTeacherLoss(nn.Module):
    ...
    def __init__(self, opt, student_for_init_teacher):
        ...
        if opt.Prediction == 'Attn':
            self.text_for_pred = torch.LongTensor(opt.batch_size).fill_(opt.sos_token_index).to(device)

        # copy student and init
        for param_t, param_s in zip(self.teacher.parameters(), student_for_init_teacher.parameters()):
    ...
    def forward(self, student_input, student_logit, student, teacher_input, iteration):
        ...
        if 'CTC' in self.opt.Prediction:
            ...
            student_logit_unlabel = student(image_unlabel, text=self.text_for_pred, is_train=False)
        ...
        with torch.no_grad():
            ...
            if 'CTC' in self.opt.Prediction:
                ...
                teacher_logit = self.teacher(teacher_input, text=self.text_for_pred, is_train=False)

        softmax_student_logit = F.softmax(student_logit, dim=2)
        softmax_teacher_logit = F.softmax(teacher_logit, dim=2)

        loss_SemiSL = self.opt.MT_C * self.criterion(softmax_student_logit, softmax_teacher_logit)
