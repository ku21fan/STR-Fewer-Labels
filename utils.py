...
class CTCLabelConverter(object):
    ...
    def __init__(self, character):
        ...
        list_special_token = ['[PAD]', '[UNK]', ' ']     # [UNK] for unknown character, ' ' for space.
    ...
    def encode(self, word_string, batch_max_length=25):
        ...
        word_index = torch.LongTensor(len(word_string), batch_max_length).fill_(self.dict['[PAD]'])

        for i, word in enumerate(word_string):
            word = list(word)
            word_idx = [self.dict[char] if char in self.dict else self.dict['[UNK]'] for char in word]
...
class AttnLabelConverter(object):
    ...
    def __init__(self, character):
        ...
        list_special_token = ['[PAD]', '[UNK]', '[SOS]', '[EOS]', ' ']  # [UNK] for unknown character, ' ' for space.
        list_character = list(character)
    ...
    def encode(self, word_string, batch_max_length=25):
        ...
        """
        word_length = [len(word) + 1 for word in word_string]  # +1 for [EOS] at end of sentence.
        batch_max_length += 1

        # additional batch_max_length + 1 for [SOS] at first step.
        word_index = torch.LongTensor(len(word_string), batch_max_length + 1).fill_(self.dict['[PAD]'])
        ...
        for i, word in enumerate(word_string):
            ...
            word.append('[EOS]')
            word_idx = [self.dict[char] if char in self.dict else self.dict['[UNK]'] for char in word]
            word_index[i][1:1 + len(word_idx)] = torch.LongTensor(word_idx)  # word_index[:, 0] = [SOS] token
...
def adjust_learning_rate(optimizer, iteration, opt):
    ...
    for milestone in opt.schedule:
        lr *= opt.lr_drop_rate if iteration >= (float(milestone) * opt.num_iter) else 1.
