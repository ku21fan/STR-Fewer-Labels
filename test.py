...
def benchmark_all_eval(model, criterion, converter, opt, calculate_infer_time=False):
    ...
    if opt.eval_type == 'benchmark':
        ...
        eval_data_list = ['IIIT5k_3000', 'SVT', 'IC13_1015', 'IC15_2077', 'SVTP', 'CUTE80']
        ...
        eval_data_list = ['5.COCO', '6.RCTW17', '7.Uber',
                          '8.ArT', '9.LSVT', '10.MLT19', '11.ReCTS']
    ...
    if opt.log_multiple_test:
        ...
        log_all_model = open(f'./evaluation_log/log_all_model_evaluation_{today}.txt', 'a')
        log_all_model.write(f'./result/{opt.exp_name}\tTotal_accuracy:{total_accuracy:0.2f}\n')
        log_all_model.write('\t'.join(accuracy_list) + '\n')
    log_all_model.close()
...
def validation(model, criterion, eval_loader, converter, opt, tqdm_position=1):
    ...
    for i, (image_tensors, labels) in tqdm(enumerate(eval_loader), total=len(eval_loader), position=tqdm_position, leave=False):
        ...
        if 'CTC' in opt.Prediction:
            ...
            cost = criterion(preds.log_softmax(2).permute(1, 0, 2), labels_index, preds_size, labels_length)

        else:
            text_for_pred = torch.LongTensor(batch_size).fill_(converter.dict['[SOS]']).to(device)
    ...
    return valid_loss_avg.val(), score, preds_str, confidence_score_list, labels, infer_time, length_of_data
...
def test(opt):
    ...
    try:
        ...
        print('*** pretrained model not match strictly *** and thus load_state_dict with strict=False mode')
    ...
    model.eval()
    with torch.no_grad():
        if opt.eval_type:  # evaluate 6 benchmark evaluation datasets or 7 additionally collected evaluation datasets
